...
test("Registry Catalog has expected counts", () => {
  ...
  assert.equal(Object.keys(SKILLS).length, 108, "Expected exactly 108 Specialist Skills");
...
test("searchRegistry finds relevant items", () => {
  ...
  assert.equal(emptyQueryResults.length, 122, "Empty query should return all 122 catalog items");
